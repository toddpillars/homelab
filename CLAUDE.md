...
| cloudflared | cloudflared | Deployment | Cloudflare Tunnel (2 replicas); under `infrastructure/controllers/`; tunnel credentials in `tunnel-secret.enc.yaml`. **Ingress is dashboard-managed** (Cloudflare Zero Trust → the tunnel's "Published application routes" tab); the `ingress:` block in the ConfigMap is a mirror-only doc and is **ignored at runtime**. Add/change tunnel hosts in the dashboard, not git (cloudflared auto-reloads, no restart). |
| n8n | naten | Deployment | Workflow automation; under `infrastructure/controllers/`; exposed at n8n.toddpillars.com |
| open-webui | open-webui | HelmRelease | LLM chat UI; under `infrastructure/controllers/`; connects to a LAN Ollama instance. **Internet-exposed** at ais.toddpillars.com through the cloudflared tunnel (tunnel-only, no Traefik ingress — same pattern as n8n). |
| postiz | postiz | HelmRelease | Social-media scheduler (v2); under `infrastructure/controllers/`. Vendored PR #19 chart in `base/postiz/chart/`, sourced from the `flux-system` GitRepository (Bitnami subcharts removed), `reconcileStrategy: Revision` since the chart is edited in place. Self-hosted Postgres (**major pinned to 16** — Renovate blocked from major bumps) + Valkey + Temporal (`auto-setup`, required by v2) + Temporal UI as plain manifests. App image pinned + a startup `command`/`args` sed strips un-grantable LinkedIn scopes. Secrets in `postgres-secret.enc.yaml` / `postiz-secrets-ext.enc.yaml`. **HTTPS** (LAN-only) at postiz.toddpillars.com via a Let's Encrypt `postiz-tls` cert (see cert-manager); console at temporal.toddpillars.com (HTTPS via a `temporal-tls` cert, LAN-only, not in the cloudflared tunnel). Datastores + Temporal scraped via ServiceMonitors in `monitoring`. See `docs/operations/postiz.md` |
| garage | garage | HelmRelease | S3-compatible object store; under `infrastructure/controllers/`. Chart sourced via a dedicated `GitRepository` pinned to a release tag (Garage has no hosted Helm repo index), not vendored. Single instance (`replicaCount: 1`, `replicationFactor: 1`) — matches the single k3s node. `rpcSecret` is chart-generated, not SOPS-managed. Cluster layout + bucket/key creation are manual `garage` CLI steps via `kubectl exec` (not declarative) — see `docs/operations/garage.md`. **HTTPS** (LAN-only) at s3.toddpillars.com via a Let's Encrypt `garage-tls` cert (see cert-manager). Scraped via a ServiceMonitor in `monitoring`. |
| cert-manager | cert-manager | HelmRelease | TLS cert automation; under `infrastructure/controllers/base/cert-manager/`. `ClusterIssuer`s `letsencrypt-staging` / `letsencrypt-prod` solve **DNS-01 via Cloudflare** (token in `cloudflare-token.enc.yaml`), so LAN-only hosts can still get publicly-trusted certs. Consumers: postiz, temporal, and garage (infrastructure/-tree — explicit `Certificate` resources), plus mealie/mls, audiobookshelf-internal/absi, and homepage/hms (apps/-tree — the `cert-manager.io/cluster-issuer` **ingress-shim annotation**, so no cert-manager CRD lands in the apps/ Kustomization, which reconciles before infrastructure/). |
...
- Services are `ClusterIP`. Internet-exposed hosts go through the Cloudflare Tunnel (routes are **dashboard-managed** → Service, HTTPS terminated at the edge). LAN-only hosts use a Traefik ingress and get publicly-trusted TLS from cert-manager (Let's Encrypt via Cloudflare DNS-01)
- cert-manager TLS pattern: `apps/`-tree hosts use the `cert-manager.io/cluster-issuer` ingress-shim annotation on the Ingress (keeps the cert-manager CRD out of the apps/ Kustomization, which reconciles before infrastructure/); `infrastructure/`-tree hosts use an explicit `Certificate` resource
