...
| cert-manager | cert-manager | HelmRelease | TLS cert automation; under `infrastructure/controllers/base/cert-manager/`. `ClusterIssuer`s `letsencrypt-staging` / `letsencrypt-prod` solve **DNS-01 via Cloudflare** (token in `cloudflare-token.enc.yaml`), so LAN-only hosts can still get publicly-trusted certs. Consumers: postiz and temporal (infrastructure/-tree — explicit `Certificate` resources), plus mealie/mls, audiobookshelf-internal/absi, and homepage/hms (apps/-tree — the `cert-manager.io/cluster-issuer` **ingress-shim annotation**, so no cert-manager CRD lands in the apps/ Kustomization, which reconciles before infrastructure/). |
